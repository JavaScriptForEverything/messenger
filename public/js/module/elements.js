...
/* element roles : Don't create everything in single function
		function-1: return element from html
		function-2: update single place in UI 
		function-3: Component: update specific task by this function
*/
			...
			<div name='label-container' class='flex-1 flex flex-col justify-center pointer-events-none'>
				...
				<div name='message-container' class=' relative flex items-center justify-between'>
					...
					'>
						...
						<span name='notification-value' class='hidden px-1.5 rounded-full text-[length:12px] bg-blue-500 text-slate-50 absolute right-0 -top-1'> ${notificationValue} </span> 
					</p>
				</div>
			</div>
...
// => dropList({ fileName: 'filename.txt', fileSize: '5.5 MB', })
...
export const dropList = (options = {}) => {
	...
	const htmlString =`
		<div name='drop-list' class='group flex flex-col active'>
			...
			<div class='relative hidden group-[&.active]:flex group-[&.active.done]:hidden'>
				<!-- .active class change color from blue to white: if value >= 50% then active -->
				<progress name='progress-meter' min=0 max=100 value=0 class='w-full h-3 peer '></progress>
				<span name='progress-value'
					class='absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-xs text-blue-700 peer-[&.active]:text-slate-50'
				> 50% </span>
			</div>
		</div>
